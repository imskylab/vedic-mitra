...
- [x] [Give festivals and observances stable identities](https://github.com/imskylab/vedic-mitra/issues/225).
      `FestivalKind`, and the glossary is keyed on it rather than on the name an item is *displayed*
      with — the last of the four instances of
      [A claim is not its own key](knowledge-standards.md#a-claim-is-not-its-own-key), and the last
      one blocking a translated glossary
