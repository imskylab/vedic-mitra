 ...
 *
 * The whole row is the target, not just the button: a 20dp circle is a poor tap target and an
 * awkward one to reach with a screen reader. `RadioButton` therefore takes `onClick = null` — it
 * draws the state and nothing more, while `selectable` on the row carries the click and the
 * [Role.RadioButton] semantics, so the row is announced as a radio button rather than as a button
 * next to some text.
 *
 * The **caller** must wrap the group in `Modifier.selectableGroup()`. Without it a screen reader
 * announces each row's position in isolation instead of "2 of 3", which is most of what makes a
 * radio group navigable.
 */
@Composable
    ...
    Row(
        modifier =
            modifier
                ...
                .padding(vertical = RadioRowPadding),
        verticalAlignment = Alignment.CenterVertically,
    ) {
...
private val RadioRowPadding = 10.dp
